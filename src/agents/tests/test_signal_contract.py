...
def test_the_checked_in_examples_are_where_the_test_expects_them():
    ...
    assert len(list(EXAMPLES.glob("*.json"))) == 11
...
class TestTheModelsAndTheSchemaAgree:
    ...
    def test_the_horizon_reaches_exactly_as_far_on_both_sides(self):
        ...
        #
        # Both bounds, not only the cap. The lower one is the less interesting number and
        # exactly the sort that drifts unwatched, and comparing what pydantic *emits* against
        # the file is a stronger check than comparing a constant: it catches the field being
        # changed without the constant, which is a different mistake from the constant being
        # changed at all.
        contract = schema()["properties"]["horizon_days"]
        emitted = TradeSignal.model_json_schema()["properties"]["horizon_days"]

        assert contract["maximum"] == MAX_HORIZON_DAYS
        assert (emitted["minimum"], emitted["maximum"]) == (
            contract["minimum"],
            contract["maximum"],
        )
...
class TestAnAnswerThatIsNotTheContractIsRefused:
    ...
    def test_a_horizon_of_no_days_is_refused(self):
        payload = example("signal-buy.json") | {"horizon_days": 0}
